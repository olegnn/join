...
use super::super::expr_chain::expr::{DefaultActionExpr, ProcessActionExpr, ProcessExpr};
use super::super::expr_chain::{
    is_block_expr, Chain, ExprChainWithDefault, ExtractExpr, ProcessWithDefault, ReplaceExpr,
};
...
pub fn generate_union(
    ...
    let futures_crate_path = if futures_crate_path.is_some() {
        if !is_async {
            panic!("futures_crate_path should be only provided for `async` `union!`")
        } else {
            futures_crate_path.unwrap()
        }
    } else {
            ...
            chains
                ...
                .map(|(chain_index, chain_step_actions)| match chain_step_actions {
                    Some(chain) => chain
                        ...
                        .unwrap_or((empty_stream.clone(), empty_stream.clone())),
                    None => {
                        let previous_result_name = get_chain_result_name(chain_index);
