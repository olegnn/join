...
impl<'a> JoinGenerator<'a> {
    ...
    pub fn generate_def_and_step_streams_for_step(
        ...
        self.chains
            ...
            .enumerate()
            .filter_map(
                |(branch_index, chain_step_actions)| match chain_step_actions {
                    Some(chain) => chain
                        .iter()
                        .enumerate()
                        .fold(
    ...
    ///
    pub fn get_step_branch_count(&self, step_number: impl Into<usize>) -> usize {
        let step_number = step_number.into();
        self.depths
            .iter()
    ...
    ///
    /// Returns true if branch with given index is active in step with provided step number.
    ///
    pub fn has_branch_in_step(
        &self,
        step_number: impl Into<usize>,
    ...
    ///
    pub fn generate_indexed_step_results_name(
        &self,
        step_number: impl Into<usize>,
        ...
        if self.get_step_branch_count(step_number) > 1 {
    ...
    ///
    /// Generates thread builders for step if `join_spawn!` or `spawn!` macro call was used.
    ///
    pub fn generate_thread_builders_and_spawn_joiners(
        &self,
        step_number: impl Into<usize>,
        ...
        if is_async || !is_spawn {
            ...
            let thread_builders = (0..self.branch_count).filter_map(|branch_index| {
                if self.has_branch_in_step(step_number, branch_index) { 
                    ...
                    let thread_index = branch_index as u16;
                    Some(quote! { let #thread_builder_name = #construct_thread_builder_fn_name(#thread_index); })
                } else {
                    None
                }
            });
            ...
            let result_joiners = (0..self.branch_count).filter_map(|branch_index| {
                if self.has_branch_in_step(step_number, branch_index) {
                    let step_result = self.generate_indexed_step_results_name(step_number, index);
                    index += 1;
            ...
            Some((
                ...
                quote! { let #current_step_results_name = (#( #result_joiners ),*); },
            ))
        }
    }
    ...
    ///
    pub fn generate_step_from_streams(
        &self,
        ...
        if is_async {
            ...
            let await_results = if self.get_step_branch_count(step_number) > 1 {
                if is_try {
            ...
            let (thread_builders, spawn_joiners) = self
                .generate_thread_builders_and_spawn_joiners(step_number)
    ...
    ///
    pub fn join_steps<TPat: ToTokens, TVar: ToTokens + Clone>(
        &self,
        step_number: impl Into<usize>,
        ...
        let extracted_results =
            self.extract_results_tuple(&current_step_results_name, &result_pats, None, step_number);
            ...
            if transpose {
                let (is_result_successful, result_vars_matcher): (Vec<_>, Vec<_>) = result_vars
                    ...
                    .filter_map(|(index, result_var)| {
                        if self.has_branch_in_step(step_number, index) {
                            (
                ...
                let current_step_results = if is_async {
                    ...
                    let ok_result_vars = (0..branch_count).filter_map(|branch_index| {
                        if self.has_branch_in_step(step_number, branch_index) {
                            let result_var =
                                self.generate_indexed_step_results_name(step_number, index);
                            index += 1;
    ...
    ///
    /// Generates `TokenStream` which contains all steps.
    ///
    pub fn generate_steps(&self) -> TokenStream {
        ...
        (0..self.max_step_count)
            .map(|step_number| {
                let (def_streams, step_streams) =
                    self.generate_def_and_step_streams_for_step(step_number);
                (
                    step_number,
                    self.generate_step_from_streams(&def_streams, &step_streams, step_number),
                )
            })
            .rev()
    ...
    ///
    pub fn extract_results_tuple<'b, TRes: ToTokens + 'b, TVar: ToTokens + 'b>(
        &self,
        results_var: &'b TRes,
        result_vars: &'b [TVar],
        ...
        let extracted = step_number.map_or_else(
            ...
            |step_number| {
                ...
                let result_vars = result_vars.iter().filter(|_| {
                    let result = self.has_branch_in_step(step_number, index);
                    index += 1;
                    result
                });
                quote! { let (#( #result_vars ),*) = #results_var; }
