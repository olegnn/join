//!
//! Definition of `join!` macro output generator.
//!
...
impl<'a> JoinGenerator<'a> {
    ...
    ///
    pub fn get_branch_result_pat(&self, branch_index: impl Into<u16>) -> TokenStream {
        let branch_index = branch_index.into() as usize;
    ...
    ///
    pub fn get_branch_result_name(&self, branch_index: impl Into<u16>) -> Ident {
        let branch_index = branch_index.into() as usize;
        self.branch_pats[branch_index]
    ...
    fn generate_indexed_step_results_name<TName: ToTokens>(
        ...
        if self.get_active_step_branch_count(step_number) > 1 {
            let index = Index::from(index as usize);
