...
struct StepAcc<'a> {
    ...
    step_streams: Vec<(TokenStream, Option<ActionExprPosition<'a>>)>,
}
...
impl<'a> JoinGenerator<'a> {
    ...
    ///
    pub fn get_branch_result_name(&self, chain_index: impl Into<usize>) -> TokenStream {
        let chain_index = chain_index.into();
        self.branch_pats[chain_index]
    ...
    ///
    pub fn wrap_previous_step_result(&self, value: TokenStream) -> TokenStream {
    ...
    fn unwrap_last_step_expr<'b, 'c>(
        &self,
        previous_def_stream: impl Into<Option<TokenStream>>,
        mut step_streams: Vec<(TokenStream, Option<ActionExprPosition<'c>>)>,
        action_expr_with_indices: impl Into<Option<ActionExprPosition<'b>>>,
    ) -> (
        Option<TokenStream>,
        Vec<(TokenStream, Option<ActionExprPosition<'c>>)>,
    ) {
        let previous_def_stream = previous_def_stream.into();

        let (previous_result, _) = step_streams.pop().expect(
            "join: Unexpected error when trying to get last step stream. This's a bug, please report it.",
        );

        let (current_expr, action_expr_wrapper) = step_streams.pop().expect("join: Step expressions length is zero while it should be >1. This's a bug, please report it.");
        ...
        let replaced_expr = action_expr_wrapper
            .expr
            .replace_inner(vec![parse_quote! { |__v| #previous_result }])
            .expect("join: Failed to replace expr in unwrap expr. This's a bug, please report it.");

        let replaced_action_expr_position = ActionExprPosition {
            expr: &replaced_expr,
            expr_index: action_expr_wrapper.expr_index,
            chain_index: action_expr_wrapper.chain_index,
        };

        let (def_stream, step_expr) = self.generate_def_and_step_expr(
            previous_def_stream,
            current_expr,
            replaced_action_expr_position,
        );

        let (def_stream, step_expr) =
            self.generate_def_and_step_expr(def_stream, step_expr, action_expr_with_indices.into());

        step_streams.push((step_expr, None));

        (def_stream, step_streams)
    }

    fn generate_def_and_step_expr<'b>(
        &self,
        previous_def_stream: impl Into<Option<TokenStream>>,
        previous_result: impl Into<Option<TokenStream>>,
        action_expr_with_indices: impl Into<Option<ActionExprPosition<'b>>>,
    ) -> (Option<TokenStream>, TokenStream) {
        let previous_def_stream = previous_def_stream.into();
        let previous_result = previous_result.into();

        if let Some(ActionExprPosition {
            expr: action_expr,
            chain_index,
            expr_index,
        }) = action_expr_with_indices.into()
        {
            ...
            (
                previous_def_stream,
                previous_result.expect(
                    "join: Unexpected previous result = `None`. This's a bug, please report it.",
                ),
            )
        }
    }
    ...
    fn handle_move_type<'b>(
        ...
        let ActionExprPosition {
            expr: action_expr,
            chain_index,
            expr_index,
        } = action_expr_with_indices.into().expect("join: Unexpected `None` ActionExprPosition in `handle_move_type`. This's a bug, please report it.");
        match action_expr.get_move_type() {
            MoveType::Unwrap => {
                if step_streams.len() > 1 {
                    let (def_stream, step_streams) = self.unwrap_last_step_expr(
                        previous_def_stream,
                        step_streams,
                        ActionExprPosition {
                            expr: action_expr,
    ...
    ///
    pub fn generate_def_and_step_streams(
        &self,
                ...
                |(chain_index, chain_step_actions)| match chain_step_actions {
                    Some(chain) => chain
                            ...
                            |acc: Option<StepAcc>,
                             (expr_index, &action_expr)| {
                                ...
                                .or_else(|| {
                                    ...
                                    let previous_result = self.wrap_previous_step_result(
                                    ...
                                    Some(
                                        StepAcc {
                                            def_stream, 
                                            step_streams: vec![(step_expr, None)]
                                        }
                                    )
                                })
                            },
                        ...
                        .map(|StepAcc { mut def_stream, mut step_streams }| loop {
                            if step_streams.len() > 1 {
                                let result = self.unwrap_last_step_expr(def_stream, step_streams, None);
                                def_stream = result.0;
                                step_streams = result.1;
                            } else {
                    ...
                    None => {
                        ...
                        (
                            None,
                            self.wrap_previous_step_result(quote! { #previous_result_name }),
                        )
                    }
                },
            )
    ...
    pub fn generate_step(
        ...
        let current_step = if is_async {
            ...
            quote! {
                #thread_builders
                #( #def_streams )*
                let #current_step_result_name = (#( #step_streams ),*);
            }
        };
...
impl<'a> ToTokens for JoinGenerator<'a> {
    fn to_tokens(&self, output: &mut TokenStream) {
        ...
        let step_by_step_results = (0..self.max_step_count)
            .map(|step_number| (step_number, self.generate_def_and_step_streams(step_number)))
