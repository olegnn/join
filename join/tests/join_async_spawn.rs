...
mod join_async_spawn_tests {
    ...
    fn it_produces_n_branches_with_any_length() {
        ...
        rt.block_on(async {
            let product = try_join_async_spawn! {
                ...
                ok_five().and_then(to_err).and_then(add_one_ok).or_else(|_| ok(5)), // 5
                and_then => |a, b, c, d| ok(a * b * c * d)
            };
    ...
    fn it_produces_n_branches_with_any_length_using_combinators() {
        ...
        rt.block_on(async {
            let product = try_join_async_spawn! {
                ...
                ok_five() |> add_one_to_ok => to_err => add_one_ok <= |_| ok(5), // 5
                map => |a, b, c, d| a * b * c * d
            };
    ...
    fn it_tests_steps() {
        ...
        rt.block_on(async {
            let product = try_join_async_spawn! {
                ...
                let branch_3 = ok_five() ~|> add_one_to_ok ~=> to_err <= |_| ok(5) ~=> add_one_ok, // 6
                map => |a, b, c, d| a * b * c * d
            };
    ...
    fn it_produces_tuple() {
        let rt = Runtime::new().unwrap();
        rt.block_on(async {
            let values = try_join_async_spawn! { ok::<_,()>(2), ok::<_,()>(3) }.await;
    ...
    fn it_produces_single_value() {
        let rt = Runtime::new().unwrap();
        rt.block_on(async {
            let value = try_join_async_spawn! { ready(Ok::<_,()>(1)) }.await;
    ...
    fn it_cant_work_without_tokio() {
        assert!(
            ::std::panic::catch_unwind(|| ::futures::executor::block_on(async {
                let failure = try_join_async_spawn! { ok::<_,()>(2u16), ok::<_,()>(1u16) };
                failure.await
            }))
            .is_err()
        );
